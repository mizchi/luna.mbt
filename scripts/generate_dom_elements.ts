...
interface GenerateFileOptions extends GeneratorOptions {
  svgOnly?: boolean; // If true, only generate SVG elements
}

function generateFile(opts: GenerateFileOptions): string {
  const relevantElements = elements.filter((e) => {
    const fnName = e.fnName ?? e.tag;
    const isSvg = e.namespace === "svg";

    // Filter by SVG/HTML
    if (opts.svgOnly && !isSvg) {
      return false;
    }
    if (!opts.svgOnly && isSvg) {
      return false;
    ...
    // Filter by target (DOM vs server_dom)
    if (opts.target === "dom") {
  ...
  const fileType = opts.svgOnly ? "SVG" : "HTML";
  const sections: string[] = [
    generateHeader(opts),
    opts.svgOnly ? "" : generateHelpers(opts), // Helpers only in main file
    `// =============================================================================`,
    `// ${fileType} Element factories`,
    "// =============================================================================",
    "",
    ...relevantElements.map((e) => generateElement(e, opts)),
    opts.svgOnly ? "" : generateTextHelpers(opts), // Text helpers only in main file
  ].filter(Boolean);
...
function generateSvgHeader(opts: GeneratorOptions): string {
  const warning = `// =============================================================================
// AUTO-GENERATED FILE - DO NOT EDIT DIRECTLY
// Generated by: scripts/generate_dom_elements.ts
// =============================================================================
...
function generateSvgFile(opts: GeneratorOptions): string {
  const relevantElements = elements.filter((e) => e.namespace === "svg");

  const sections: string[] = [
    generateSvgHeader(opts),
    "// =============================================================================",
