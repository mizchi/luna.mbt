...
export function hydrate(element, _state, name) {
  ...
  const render = () => {
    element.innerHTML = `
      <div style="display: inline-flex; align-items: center; gap: 0.75rem; padding: 1rem 1.5rem; border: 2px solid var(--primary-color, #fbbf24); border-radius: 0.75rem; background: var(--sidebar-bg, #fafafa); position: relative;">
        <span style="font-size: 0.75rem; color: var(--text-muted, #6b7280); position: absolute; top: -0.5rem; left: 1rem; background: var(--bg-color, #fff); padding: 0 0.5rem;">${name}</span>
        ...
        <span style="min-width: 3rem; text-align: center; font-weight: 700; font-size: 1.5rem; color: var(--text-color, #1f2937);">${count}</span>
        <button style="width: 2.5rem; height: 2.5rem; border: none; border-radius: 0.5rem; background: var(--primary-color, #fbbf24); color: #000; cursor: pointer; font-size: 1.5rem; font-weight: bold; transition: opacity 0.2s;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">+</button>
      </div>
    `;
