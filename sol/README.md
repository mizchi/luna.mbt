...
# or via npm
pnpm add -g @luna_ui/sol             # npm wrapper
```
...
From `sol/`, use `just bootstrap` to install dependencies and generate the
examples, then `just sol --help` or `just dev-app` to run the local CLI.
The development runner builds the CLI through the library workspace, so it
also works before example code has been generated.

`just verify` checks MoonBit, documentation, CLI installation and scaffolding.
`just test-sol-app` tests the production example; `just test-sol-app-hydration`
and `just test-mbt-e2e` start their own development server and Chromium browser.
...
`astra/src/assets/scripts/`. `sol generate` copies them into
`.sol/{dev,prod}/__sol__/`. Cloudflare development serves the generated
runtime and island scripts through a Wrangler Assets binding.

When you update the Luna loader runtime, run these commands from `sol/`:

```bash
pnpm --dir ../js/loader build
just sync-luna-assets
just check-luna-assets
```
...
- **Mars Integration**: Server routes and middleware implemented in MoonBit
